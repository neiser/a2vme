...
#define MAXLEN		132
#define BS		19
#define RANGENUMBER 2 //using memory blocks 2 and 3 instead of 0 and 1
...
void printlog(char *output, bool newline, bool verbose)
{
	if (logFile != NULL)
	{
		...
		if (newline) fprintf(logFile, "[%08d] ", (int) time(NULL) - timestamp);
...
//	printf("UpperAddr: 0x%x\n",UpperAddr);
	LowerAddr= NewAddr & 0x1fffffff;
//	printf("LowerAddr: 0x%x\n",LowerAddr);
...
// 					if (verbose) printf("time: %f clocks\n", (int) time(NULL) - timestamp); //timestamp = (int) clock();
			...
			else
			{
				if (!quiet)
				{
					int i;
					printf("\r");
					for (i = 0; i < line / 10000; i++) printf(">");
					for (i = line / 10000; i < 38; i++) printf("-");
					fflush(stdout);
				}
			}
		}
		for(i=0;i<len/8;i++)
		{
			...
			for(j=0;j<8;j++)
			{
				...
				//          printf("word = %x\n",l_word);
			if (!simulate) prog_flash((int)p_addr,l_word, verbose);
			p_addr++;
			l_count++;
...
main (int argc, char *argv[])
{
	...
	for (i = 1; i < argc;i++)
	{
		if(strncmp(argv[i], "-",1) == 0)
		{
			...
			{
				if(strcmp(argv[i], "--erase") == 0) {eraseFlash = true;}
				if(strcmp(argv[i], "--write") == 0) {writeFlash = true;}
				if(strcmp(argv[i], "--verify") == 0) {verifyFlash = true;}
				if(strcmp(argv[i], "--restart") == 0) {restartModule = true;}
				if(strcmp(argv[i], "--help") == 0) {displayHelp = true;}
				if(strcmp(argv[i], "--simulate") == 0) {simulate = true;}
				if(strcmp(argv[i], "--verbose") == 0) {verbose = true; quiet = false;}
				if(strcmp(argv[i], "--quiet") == 0) {quiet = true; verbose = false;}
			}
			else
			{
				if(strchr(argv[i], 'e') != 0) {eraseFlash = true;}
				if(strchr(argv[i], 'w') != 0) {writeFlash = true;}
				if(strchr(argv[i], 'y') != 0) {verifyFlash = true;}
				if(strchr(argv[i], 'r') != 0) {restartModule = true;}
				if(strchr(argv[i], 's') != 0) {simulate = true;}
				if(strchr(argv[i], 'v') != 0) {verbose = true; quiet = false;}
				if(strchr(argv[i], 'q') != 0) {quiet = true; verbose = false;}
				if(strchr(argv[i], 'h') != 0) {displayHelp = true;}
	...
	if (eraseFlash) printlog("eraseFlash ", false, verbose);
	if (writeFlash) printlog("writeFlash ", false, verbose);	
	if (verifyFlash) printlog("verifyFlash ", false, verbose);
	if (restartModule) printlog("restartModule ", false, verbose);
	if (simulate) printlog("simulate ", false, verbose);
	...
	if (restartModule)
	{
		if (memoryVerified || !verifyFlash)
		{
			...
			vmeaddr = moduleNumber * MODULE_LENGTH  ;
			vme_virt_addr  = (unsigned long)openvme((void *)vmeaddr, VME_LENGTH, verbose);
			if (!simulate) restart(vme_virt_addr, 0+RANGENUMBER);
			closevme(verbose);
